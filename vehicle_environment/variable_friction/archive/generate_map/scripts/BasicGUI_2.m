function varargout = BasicGUI_2(varargin)
%%%%%READ ME%%%%%%%%
%Specific Mode: es werden nur ganz bestimmte Farben unterstützt!
%Legende:  gelb: ffff00 (html-Notation)
%          blau: 0000ff
%          grün: 00ff00
%          rot:  ff0000
% Edit the above text to modify the response to help BasicGUI_2

% Last Modified by GUIDE v2.5 26-Jan-2019 14:50:39

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @BasicGUI_2_OpeningFcn, ...
                   'gui_OutputFcn',  @BasicGUI_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BasicGUI_2 is made visible.
function BasicGUI_2_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for BasicGUI_2
...
set(findall(handles.specific_panel, '-property', 'enable'),'enable', 'off');    %Disable Specific_panel
...
function varargout = BasicGUI_2_OutputFcn(hObject, eventdata, handles) 
varargout{1} = handles.output;

% --- Executes on button press in global_button.
function global_button_Callback(hObject, eventdata, handles)
set(handles.specific_value_pathname, 'String', '');                             %Set specific value pathname to none
...
set(handles.browse_button, 'Enable', 'Inactive');                               %Disable specific browser button
...
% --- Executes on button press in specific_button.
function specific_button_Callback(hObject, eventdata, handles)
set(handles.browse_button, 'Enable', 'On');                                     %Enable specific browser button
set(handles.raceline_pathname, 'String', '');                                   %Set raceline pathname to none
set(handles.global_text,'String','');                                           %Set text above "global slider" to none
set(handles.random_text1,'String','');                                          %Set text above "random slider1" to none
set(handles.random_text2,'String','');                                          %Set text above "random slider2" to none
...
set(findall(handles.random_panel1, '-property', 'enable'),'enable', 'off');     %Disable Random_panel1
set(findall(handles.random_panel2, '-property', 'enable'),'enable', 'off');     %Disable Random_panel2
set(findall(handles.specific_panel, '-property', 'enable'),'enable', 'on');     %Enable Specific_panel
set(findall(handles.raceline_panel, '-property', 'enable'),'enable', 'off');    %Disable Raceline_panel
...
[filename, pathname] = uigetfile('*.pgm', 'Pick a .pgm file');                      %Browse for file
handles.pathname_map_specific = strcat(pathname, filename);                         %Connect path name and file name to one string 
data_type=lower(filename(end-2:end));                                               %Extract data type and convert to string
if data_type =='pgm'                                                 %#ok<*BDSCA>
    set(handles.specific_value_pathname, 'String', handles.pathname_map_specific);  %Set GUI.full_path to string fullpathname
else
    set(handles.specific_value_pathname, 'String', 'ERROR');                        %Display "ERROR"
end
guidata(hObject, handles);
...
function map_name_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function map_name_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
if get(handles.checkbox, 'Value')==1                                                %If Checkbox for Raceline Percentage Mode is activated
    ...
    if d>-99 && d<99                                                                    %If Raceline Percentage Value is consistent
        ...
        if strcmp(path_percent, "") || strcmp(path_percent,'ERROR')                         %If Pathname is not consistent
            ...
            if get(handles.specific_button,'Value')                                             %If Specific_Button is activated 
                set(handles.specific_value_pathname, 'String', 'ERROR');                            %Set Specific Value Pathname to "ERROR"
            end   
...
if a==1
    c=get(handles.map_name,'String');
    assignin('base','map_name',c); 
    if get(handles.global_button,'Value')                                   %If Global_Button is activated
        ...
        close(BasicGUI_2);                                                      %Close BasicGUI_2
    ...
    elseif get(handles.specific_button,'Value')                             %If Specific_Button is activated
        path = get(handles.specific_value_pathname, 'String');                  %Get Specific Value Pathname
        if strcmp(path, "")                                                     %If Specific Value Pathname is none
            set(handles.specific_value_pathname, 'String', 'ERROR');                %Set Specific Value Pathname to "ERROR"
        elseif strcmp(path,'ERROR')                                             %If Specific Value Pathname is "ERROR"
            set(handles.specific_value_pathname, 'String', 'ERROR');                %Set Specific Value Pathname to "ERROR"
        else                                                                    %else (if Specific Value Pathname is consistent)
            map_specific = importdata(handles.pathname_map_specific);               %Import Map Specific
            assignin('base', 'map_specific', map_specific);                         %Handover Map Specific to base workspace
            assignin('base', 'mode', 'specific');                                   %Handover "mode" to base workspace
            close(BasicGUI_2);                                                      %Close BasicGUI_2
            Friction_Map_Creation();                                                %Run Friction_Map_Creation
        end   
